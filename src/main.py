...
def _train_and_evaluate(df: pd.DataFrame, target_col: str = 'efficiency'):
    ...
    X_train, y_train, scaler, label_encoders = preprocess_data(train_df, is_test=False)
    ...
    X_test, y_test, _, _ = preprocess_data(
        test_df,
        is_test=False,  # keep efficiency to extract target
...
if __name__ == "__main__":
    try:
        ...
        print("data_loader.py not found, using dummy data...")
        ...
        # Run the training and evaluation process
        trained_model, predictions, mae, rmse, r2 = _train_and_evaluate(df)
        print("\nScript completed successfully with dummy data.")
        
    except Exception as e:
        print(f"Error: {e}")
        print("Please ensure your data files contain the required columns: 'id' and 'efficiency'")  
